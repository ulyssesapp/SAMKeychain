...
- (BOOL)deleteItem:(NSError *__autoreleasing *)error {
	if (!self.service || !self.account) {
		if (error) {
			*error = [[self class] errorWithCode:SAMKeychainErrorBadArguments];
		}
		return NO;
	}
...
- (void)setPasswordObject:(id<NSCoding>)object {
	self.passwordData = [NSKeyedArchiver archivedDataWithRootObject:object requiringSecureCoding:NO error:NULL];
}
